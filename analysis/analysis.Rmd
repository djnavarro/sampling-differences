...
```{r}
exp2_classification <- subgroup_individual %>% 
  dplyr::filter(exp == "exp2") %>% 
  dplyr::group_by(id) %>% 
  dplyr::summarise(
    age = dplyr::first(age),
...
readr::write_csv(
  exp2_classification,
  here::here("analysis", "exp2_classification.csv")
)

print(exp2_classification, n = 100)
```

Note that this table is also saved in the "analysis" folder of the repository, as "exp2_classification.csv"
